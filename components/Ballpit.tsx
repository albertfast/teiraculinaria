...
class X {
  ...
  // store bound handlers so removeEventListener works reliably
  ...
  #initObservers() {
    if (!(this.#config.size instanceof Object)) {
      ...
      if (this.#config.size === 'parent' && this.canvas.parentNode) {
        this.#resizeObserver = new ResizeObserver(this.#boundOnResize as any);
...
const XDefaults = {
  count: 200,
  // Warm amber gradient (can be overridden via props)
  colors: [0x7c2d12, 0xf59e0b, 0xffe4b5],
  ambientColor: 0x0b1220,
...
class Z extends InstancedMesh {
  config: typeof XDefaults;
  physics: W;
  ambientLight: AmbientLight | undefined;
  light: PointLight | undefined;

  constructor(renderer: WebGLRenderer, params: Partial<typeof XDefaults> = {}) {
    ...
    this.#setupLights();
    this.setColors(config.colors as any);
  }

  #setupLights() {
    this.ambientLight = new AmbientLight(this.config.ambientColor as any, this.config.ambientIntensity as any);
    (this as any).add(this.ambientLight);
    this.light = new PointLight((this.config.colors as any)[0], this.config.lightIntensity as any);
  ...
  update(deltaInfo: { delta: number }) {
    ...
    for (let idx = 0; idx < (this as any).count; idx++) {
      ...
      if (idx === 0 && (this.config as any).followCursor === false) {
        U.scale.setScalar(0);
      } else {
...
function createBallpit(canvas: HTMLCanvasElement, config: any = {}): CreateBallpitReturn {
  ...
  const pointerData = createPointerData({
    ...
    onMove() {
      ...
      (spheres.config as any).controlSphere0 = true;
    },
    onLeave() {
      (spheres.config as any).controlSphere0 = false;
    }
  });
  function initialize(cfg: any) {
    ...
    spheres = new Z(threeInstance.renderer, { ...XDefaults, ...cfg });
  ...
  threeInstance.onAfterResize = size => {
    (spheres.config as any).maxX = size.wWidth / 2;
    (spheres.config as any).maxY = size.wHeight / 2;
  };
  return {
    three: threeInstance,
    get spheres() {
